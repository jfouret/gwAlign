...
readStat=function(path){
  statFile=paste(path,"default_filter/stat_raw.txt",sep='/')
...
readDiscard=function(path){
  discardFile=paste(path,"default_filter/discard.txt",sep='/')
...
readStatFilter=function(path){
  statFile=paste(path,"default_filter/stat_filtered.txt",sep='/')
  if (file.exists(statFile)){
    tmp=read.delim(statFile,header=T,as.is=T)
    if (dim(tmp)[1]==0){
      tmp=NULL
